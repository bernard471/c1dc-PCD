...
export default function RecommendedActionsLayout({ children }: { children: React.ReactNode }) {
  const router = useRouter();
  const pathname = usePathname();
  const [isSidebarOpen, setIsSidebarOpen] = useState(true);
  const [activeSection, setActiveSection] = useState('recommended-actions');
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);
  const [isTabletOrMobile, setIsTabletOrMobile] = useState(false);
  
  // Determine if we're on a recommended actions page
  const isRecommendedActionsPage = pathname === '/recommended-actions' || pathname.startsWith('/recommended-actions/');
  ...
  const handleSectionChange = (section: string) => {
    ...
    // If we're on a recommended actions page, navigate away to the dashboard area
    if (isRecommendedActionsPage) {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="flex">
        ...
        <main className={`flex-1 transition-all duration-300 ${isSidebarOpen ? 'md:ml-64' : 'md:ml-20'}`}>
          {/* If we're on a recommended actions page, render the recommended actions content, otherwise render the section content */}
          {isRecommendedActionsPage ? (
            children
          ) : (
            <div className="p-6 md:p-8 bg-white rounded-lg shadow-md mt-4">
              {renderContent()}
            </div>
          )}
        </main>
